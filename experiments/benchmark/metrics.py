...
def plot_metrics(xs, df, coordinate, models, metric_name, gap=250):
    ...
    for model in models:
        results = df.loc[coordinate, model, :, metric_name]
    ...
    fig.tight_layout()
    fig.savefig(
        f"{image_path}/{coordinate}_{metric_name}.pdf",
        format="pdf",
        bbox_inches="tight",
    )
    plt.close(fig)
...
def main():
    ...
    #             mean_std = join_mean_std(mean, std)
    #             # print(f"{metric_name:10}  {mean_std}")
